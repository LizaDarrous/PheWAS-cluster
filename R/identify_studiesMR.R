...
identify_studiesMR <- function(ZMatrix, MR_shrinkage, MR_threshold, stepwise_threshold, Z_matrices, save_files=FALSE, verbose=FALSE){
  ...
  for(one_study in Prior_study_names) {
    ...
    #one_study_text=paste0('`',one_study,'`')
    ZMatrix %>%
      filter(abs(eval(parse(text=paste0('`',one_study,'`')))) > Zlimit) -> ZMatrix_uni
  ...
  ## ADDED LIZA
  all_uni_coefs$nm = gsub('`','',all_uni_coefs$nm)
  ...
  ### errors out cause names now surrounded by ``, might need a gsub 
